...
```{r}
basic_html |> html_elements(xpath = "//p")
```

* `.title` selects all elements that are of `class` "title"

```{r}
basic_html |> html_elements(xpath = "//title")
```
...
```{r}
read_html("https://en.wikipedia.org/wiki/Elfie_Caroline_Huntington") |> 
  html_elements("#Personal_life , #Collections, #Style, #Career, #Early_life") |> 
  html_text()
```


## Recap: Scraping HTML pages with `rvest`
...
html |> html_element(xpath = "//p") |> html_text() |> writeLines()
```
...
You can run this in a for-loop. For the loop to run efficiently, space should be pre-allocated for every object (i.e., you create a list beforehand, and its length can be determined by an educated guess). Please take breaks between requests to not put too much of a burden on the servers.
...
You can of course also just `purrr::map()` over it:

```{r}
result_list <- map(urls, read_html) 
```

### Letting the scraper navigate on its own

Extracting the link on the fly is the same thing, but in the end, you need to replace the link argument with the one you extracted. It is probably easiest to perform those things in a `while` loop, hence here is a quick revision:
...
4. ELSE: just hunt the data down with Selenium and Regexes (--> tomorrow)
...
* Selenium may be better used in Python. A couple of resources: [Basic tutorial and documentation -- you may be most interested in example 1](https://pypi.org/project/selenium/); [Extensive tutorial by geeks for geeks](https://www.geeksforgeeks.org/selenium-python-tutorial/); ChatGPT "speaks" Selenium quite well, too. 

## Exercises

1. Download the links and names of the  [History TV shows](https://www.imdb.com/chart/top/?ref_=nv_mv_250). Put them in a tibble with the columns `rank` -- in numeric format (if you know regexes already), `title`, `url` to IMDb entry, `rating` -- in numeric format.
...
2. Scrape all the pages of the most popular [Film noir movies ](https://www.imdb.com/search/title/?genres=Film-Noir&explore=genres&title_type=feature&ref_=ft_movie_10) in an automated fashion. Make sure to take breaks between requests by including `Sys.sleep(2)`.

  a. Do so using running numbers in the urls (a bit tricky here). 
  b. Do so by using `session()` in a loop.
  
```{r eval=FALSE}
url_2 <- "https://www.imdb.com/search/title/?title_type=feature&genres=film-noir&start=51&explore=genres&ref_=adv_prv"
url_3 <- "https://www.imdb.com/search/title/?title_type=feature&genres=film-noir&start=101&explore=genres&ref_=adv_prv"
...
urls <- str_c("https://www.imdb.com/search/title/?title_type=feature&genres=film-noir&start=", 
      seq(1, 982, by = 50), # determine number of potential results -- 982
      "&explore=genres&ref_=adv_prv")

content <- urls |> map(read_html)
...
# extract the content (Kudos to Jakob Tures for coming up with this solution!)
movies <- vector(mode = "list", length = length(content))

for (i in seq_along(content)) {
  movies[[i]] <- content[[i]] %>%
    html_elements(css = "div.lister-item")
}

movies <- content %>% 
  unlist(recursive = FALSE) ## remove the outer level of the list -> list of 983 movies

## now extract the data for each movie
noir <- tibble(
  rank = integer(),
  title = character(),
  url = character(),
...
for (i in seq_along(movies)) {
  temp <- tibble(
    rank = movies[[i]] %>% 
      html_element(css = ".lister-item-index") %>% 
      html_text() %>% 
      parse_number(),
    title = movies[[i]] %>% 
      html_element(css = ".lister-item-header > a") %>% 
      html_text(),
    url = movies[[i]] %>% 
      html_element(css = ".lister-item-header > a") %>% 
      html_attr("href") %>% 
      str_c("https://www.imdb.com", .),
    rating = movies[[i]] %>% 
      html_element((css = ".ratings-imdb-rating > strong")) %>% ## using html_element() instead of html_elements() returns an NA when the node couldn't be found
      html_text() %>% 
      parse_double()
  )
