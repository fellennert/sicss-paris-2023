...
```{r eval=TRUE}
imdb_lr_resampled <- fit_resamples(
  imdb_lr_wf,
  imdb_folds,
...
#imdb_lr_resampled |> write_rds("imdb_lr_resampled.rds")
```
...
```{r}
lr_rs_metrics <- collect_metrics(imdb_lr_resampled)
lr_rs_predictions <- collect_predictions(imdb_lr_resampled)
```

This can also be used to create the confusion matrix by hand.

```{r}
confusion_mat <- lr_rs_predictions |> 
  group_by(id) |> 
