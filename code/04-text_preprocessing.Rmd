...
stopwords_vec_filtered <- stopwords_vec[!stopwords_vec %in% "in"]
...
3. Count the terms per party. 
  a. Do you see party-specific differences with regard to their ten most common terms (hint: use `group_by()` before using `slice_max(tf_idf, n = 10, with_ties = FALSE)`)? 
```{r}
df <- preprocessed |> 
  count(stemmed, party) |> 
  group_by(party) |> 
...
```{r}
timelines_spacy <- read_rds("https://github.com/fellennert/sicss-paris-2023/raw/main/code/timelines_spacy.rds")
...
str_extract(string, "[#][a-zA-Z0-9_]+")string <- "@FelixLennert checks if this works #sicss2023"
