...
imdb_ft_recipe <- recipe(sentiment ~ text, data = imdb_train) |> 
  ...
  step_word_embeddings(text, embeddings = fasttext_tbl) 
...
best_match("Macron", 10)
best_match("immigration", 10)
```
...
animal_query <- c("horse", "cat", "rat", "duck", "chimpanzee", "platypus", 
             "capybara", "octopus", "dolphin", "fox", "goose", "pigeon",
             "spider", "otter", "human")
...
animal_pca <- prcomp(fasttext_mat[animal_query, ]) |> 
  pluck("x") |> 
  as_tibble(rownames = NA) |> 
  rownames_to_column("name")
...
animal_pca |> 
  ggplot() +
  geom_label(aes(PC1, PC2, label = name))
```
...
male_text <- read_rds("male_text.rds")
female_text <- read_rds("female_text.rds")
```
