...
1. Read in the Start a session with the tidyverse Wikipedia page. Adapt your user agent to some sort of different value. Proceed to Hadley Wickham's page. Go back. Go forth. Jump to a different Wikipedia page. Check the `session_history()` to see if it has worked.
...
2. Start a session on "https://www.scrapethissite.com/pages/advanced/?gotcha=login", fill out, and submit the form. Any value for login and password will do. (Disclaimer: you have to add the URL as an "action" attribute after creating the form, see [this thread](https://github.com/tidyverse/rvest/issues/319). -- so after extracting and setting the form, include and run `login_form$action <- url`) before submitting it.
